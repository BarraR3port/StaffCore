...
package cl.bebt.staffcore.PersistentData;
...
public class PersistentDataUtils {
    ...
    public static void save( String string , Object value , ItemStack itemStack , UUID ownerUUID , PersistentDataType type ){
        ...
        PersistentData persistentData = new PersistentData( values , itemStack.serialize( ).toString( ) , UUID.randomUUID( ) , ownerUUID , type );
    ...
    public static String getString( ItemStack itemStack , String key ){
        if ( has( itemStack , key , PersistentDataType.STRING) ) {
            Bukkit.broadcastMessage( ItemsData.values( ).toString() );
            for ( PersistentData item : ItemsData.values( ) ) {
                Bukkit.broadcastMessage( "strings:" + item.getStringValues().toString() );
                Bukkit.broadcastMessage( "uuid:" + item.getOwnerUUID().toString() );
    ...
    public static void saveItems( ){
        try {
            Gson gson = new Gson( );
            File file = new File( main.plugin.getDataFolder( ).getAbsolutePath( ) + "/items.json" );
