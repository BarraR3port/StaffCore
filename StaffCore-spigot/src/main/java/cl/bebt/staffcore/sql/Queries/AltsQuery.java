...
public class AltsQuery {
    ...
    public static void createAlts( User user , String ip ){
        ...
        while (intento < 4) {
            try {
                ...
                main.playerSkins.put( user.getName( ) , user.getSkin( ) );
    ...
    public static void addIps( String uuid , String Ips ){
        ...
        while (intento < 4) {
            try {
                PreparedStatement statement = Mysql.getConnection( ).prepareStatement( "UPDATE sc_alts SET Ips=? WHERE UUID=?" );
    ...
    public static void addAlts( JSONObject json ){
        ...
        for ( int i = 1; i <= length; i++ ) {
            ...
            String Skin = Http.getHead( "http://localhost:82/api/head/" + Name , Name );
            try {
    ...
    public static String getNameByUUID( UUID uuid ){
        String name = "";
        try {
            PreparedStatement statement = Mysql.getConnection( ).prepareStatement( "SELECT Name FROM sc_alts Where UUID=(?)" );
    ...
    public static UUID getUUIDByName( String name ){
        try {
            PreparedStatement statement = Mysql.getConnection( ).prepareStatement( "SELECT UUID FROM sc_alts Where Name=(?)" );
            statement.setString( 1 , name );
            ResultSet results = statement.executeQuery( );
            if ( results.next( ) ) {
