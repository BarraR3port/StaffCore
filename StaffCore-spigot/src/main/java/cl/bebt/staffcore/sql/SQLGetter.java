...
public class SQLGetter {
    ...
    public static void createStaffCoreSettingsTable( ){
        PreparedStatement ps;
        try {
            ps = Mysql.getConnection( ).prepareStatement( "CREATE TABLE IF NOT EXISTS sc_settings ( ServerName VARCHAR(30) NOT NULL DEFAULT ?, Version VARCHAR(5) NOT NULL DEFAULT '4.5.0', IsUpdated BOOLEAN NOT NULL DEFAULT true)" );
        ...
        Bukkit.getScheduler( ).runTaskAsynchronously( plugin , ( ) -> {
            ...
            if ( !AltsQuery.checkAltsTable( ) ) {
                for ( Player p : Bukkit.getOnlinePlayers( ) ) {
                    utils.tellHover( p , "&cHEY STAFFCORE IS MAKING SOME UPDATES IN THE DB" , "&aClick to see What's Going on" , "http://localhost:82/news/alts-migration" );
                }
