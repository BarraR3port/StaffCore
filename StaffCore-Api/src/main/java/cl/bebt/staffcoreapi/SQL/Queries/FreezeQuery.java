...
public class FreezeQuery {
    ...
    public static void enable( String player ){
        if ( AltsQuery.PlayerExists( player ) ) {
            try {
                PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "UPDATE sc_frozen SET Enabled=? WHERE Name=?" );
    ...
    public static String isFrozen( String player ){
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "SELECT Enabled FROM sc_frozen WHERE Name=?" );
