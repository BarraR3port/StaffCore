...
public class WarnsQuery {
    ...
    public static void createWarn( String warned , String warner , String Reason , String Date , String ExpDate , String Status ){
        ...
        while (intento < 4) {
            try {
                PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "INSERT INTO sc_warns (Name, Warner, Reason, Date, ExpDate, Status) VALUES (?,?,?,?,?,?)" );
                ...
                Utils.tellConsole( "&c[&5Staff Core&c] Creating the sc_warns table into the database" );
                SqlManager.createWarnsTable( );
                intento++;
            }
        }
    }
    ...
    public static JSONObject getWarnsInfo( int id ){
        ...
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "SELECT * FROM sc_warns WHERE WarnId LIKE " + id );
    ...
    public static JSONObject getPlayerWarnsInfo( String warned ){
        JSONObject json = new JSONObject( );
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "SELECT * FROM sc_warns WHERE Name LIKE ? order by WarnId" );
            statement.setString( 1 , warned );
            ResultSet rs = statement.executeQuery( );
            while (rs.next( )) {
                ...
                if ( now.after( exp_date ) ) {
                    ...
                    Utils.runAsync( ( ) -> closeWarn( WarnId ) );
                } else {
    ...
    public static List < String > getWarnedPlayers( ){
        ...
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "SELECT Name FROM sc_warns" );
    ...
    public static int getCurrentWarns( ){
        int currentWarns = 0;
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "SELECT count(*) AS CurrentWarns FROM sc_warns" );
            ResultSet rs = statement.executeQuery( );
            if ( rs.next( ) ) {
    ...
    public static Boolean isStillWarned( int id ){
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "SELECT ExpDate, Status FROM sc_warns WHERE WarnId=?" );
    ...
    public static ArrayList < Integer > getPlayerWarns( String warned ){
        ...
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "SELECT WarnId FROM sc_warns WHERE Name = ? order by WarnId" );
            statement.setString( 1 , warned );
            ResultSet rs = statement.executeQuery( );
            while (rs.next( )) {
    ...
    public static void closeWarn( int id ){
        ...
        while (intento < 4) {
            try {
                PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "UPDATE sc_warns SET Status = 'closed' WHERE WarnId=?" );
                statement.setInt( 1 , id );
                statement.executeUpdate( );
                break;
    ...
    public static void openWarn( int id ){
        ...
        while (intento < 4) {
            try {
                PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "UPDATE sc_warns SET Status = 'open' WHERE WarnId=?" );
                statement.setInt( 1 , id );
                statement.executeUpdate( );
                break;
            } catch ( SQLException ignored ) {
    ...
    public static void deleteWarn( int id ){
        ...
        while (intento < 4) {
            try {
                PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "DELETE FROM sc_warns WHERE WarnId = ?" );
                statement.setInt( 1 , id );
                statement.executeUpdate( );
                break;
            } catch ( SQLException ignored ) {
    ...
    public static void deleteWarns( String name ){
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "DELETE FROM sc_warns WHERE Name = ?" );
    ...
    public static ArrayList < Integer > getWarnIds( String p ){
        ...
        try {
            PreparedStatement statement = SqlUtils.getConnection( ).prepareStatement( "SELECT * FROM sc_warns" );
            ResultSet rs = statement.executeQuery( );
            while (rs.next( )) {
