...
public class closedBansMenu extends PaginatedMenu{
    ...
    public void handleMenu( InventoryClickEvent e ){
        ...
        for ( int id = 0; id <= count( ); ) {
            ...
            try {
                if ( utils.mysqlEnabled( ) ) {
                    ...
                    if ( plugin.baned.getConfig( ).getString( "bans." + id + ".status" ).equals( "closed" ) ) {
                        num++;
                        bans.put( num , id );
                    }
                }
        ...
        if ( e.getCurrentItem( ).getItemMeta( ).getPersistentDataContainer( ).has( new NamespacedKey( plugin , "closed" ) , PersistentDataType.STRING ) ) {
            p.closeInventory( );
            String jugador = e.getCurrentItem( ).getItemMeta( ).getDisplayName( );
            if ( e.getClick( ).isLeftClick( ) ) {
                int id = e.getCurrentItem( ).getItemMeta( ).getPersistentDataContainer( ).get( new NamespacedKey( plugin , "closed-id" ) , PersistentDataType.INTEGER );
            ...
            if ( bans != null && !bans.isEmpty( ) ) {
                for ( int i = 1; i <= getMaxItemsPerPage( ); i++ ) {
                    ...
                    if ( bans.get( index ) != null ) {
                        ...
                        if ( plugin.getConfig( ).getBoolean( "mys" + "ql.enabled" ) ) {
                            Date now = new Date( );
                            ...
                            if ( SQLGetter.getBaned( bans.get( index ) , "Status" ).equals( "closed" ) ) {
                            ...
                            meta.setLore( lore );
                            meta.getPersistentDataContainer( ).set( new NamespacedKey( main.plugin , "closed-id" ) , PersistentDataType.INTEGER , bans.get( index ) );
                            ...
                            lore.add( utils.chat( "&7Baned by: &a" + plugin.baned.getConfig( ).getString( "bans." + bans.get( index ) + ".baned_by" ) ) );
                            lore.add( utils.chat( "&7Reason: &b" + plugin.baned.getConfig( ).getString( "bans." + bans.get( index ) + ".reason" ) ) );
                            lore.add( utils.chat( "&7Created date: &a" + plugin.baned.getConfig( ).getString( "bans." + bans.get( index ) + ".date" ) ) );
