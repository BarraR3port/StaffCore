...
public class closedBansMenu extends PaginatedMenu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "closed"), PersistentDataType.STRING)) {
            p.closeInventory();
            String jugador = e.getCurrentItem().getItemMeta().getDisplayName();
            if (e.getClick().isLeftClick()) {
                int i = ((Integer)e.getCurrentItem().getItemMeta().getPersistentDataContainer().get(new NamespacedKey((Plugin)this.plugin, "closed-id"), PersistentDataType.INTEGER)).intValue();
                ...
                for (int i = 1; i <= getMaxItemsPerPage(); i++) {
                    ...
                    if (bans.get(Integer.valueOf(this.index)) != null) {
                        ...
                        if (this.plugin.getConfig().getBoolean("mysql.enabled")) {
                            ...
                            if (SQLGetter.getBaned(((Integer)bans.get(Integer.valueOf(this.index))).intValue(), "Status").equals("closed")) {
                                lore.add(utils.chat("&7Status: &cClosed"));
                            } else if (API.isStillBaned(((Integer)bans.get(Integer.valueOf(this.index))).intValue()).booleanValue()) {
                            ...
                            meta.setLore(lore);
                            meta.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "closed-id"), PersistentDataType.INTEGER, bans.get(Integer.valueOf(this.index)));
                            meta.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "closed"), PersistentDataType.STRING, "closed");
                            ...
                            lore.add(utils.chat("&7Baned by: &a" + this.plugin.bans.getConfig().getString("bans." + bans.get(Integer.valueOf(this.index)) + ".baned_by")));
                            lore.add(utils.chat("&7Reason: &b" + this.plugin.bans.getConfig().getString("bans." + bans.get(Integer.valueOf(this.index)) + ".reason")));
                            lore.add(utils.chat("&7Created date: &a" + this.plugin.bans.getConfig().getString("bans." + bans.get(Integer.valueOf(this.index)) + ".date")));
