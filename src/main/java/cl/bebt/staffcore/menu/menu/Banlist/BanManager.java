...
public class BanManager extends Menu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "openBans"), PersistentDataType.STRING)) {
            p.closeInventory();
            (new openBansMenu(main.getPlayerMenuUtility(p), this.plugin)).open(p);
            e.setCancelled(true);
        } else if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "closeBans"), PersistentDataType.STRING)) {
            p.closeInventory();
    ...
    public void setMenuItems() {
        ...
        c_meta.setLore(lore);
        lore.clear();
        o_meta.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "openBans"), PersistentDataType.STRING, "openBans");
        c_meta.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "closeBans"), PersistentDataType.STRING, "closeBans");
