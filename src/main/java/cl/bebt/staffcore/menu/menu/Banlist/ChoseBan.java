...
public class ChoseBan extends ReportMenu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "delete_ban"), PersistentDataType.STRING)) {
            p.closeInventory();
            BanPlayer.unBan((CommandSender)p, this.Id);
            e.setCancelled(true);
        } else if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "close_ban"), PersistentDataType.STRING)) {
            p.closeInventory();
            CloseBan(p);
            e.setCancelled(true);
    ...
    public void setMenuItems() {
        ...
        delete_meta.addItemFlags(new ItemFlag[] { ItemFlag.HIDE_ENCHANTS });
        delete_meta.addItemFlags(new ItemFlag[] { ItemFlag.HIDE_ATTRIBUTES });
        closeBan_meta.addEnchant(Enchantment.DURABILITY, 1, true);
        closeBan_meta.addItemFlags(new ItemFlag[] { ItemFlag.HIDE_ENCHANTS });
        closeBan_meta.addItemFlags(new ItemFlag[] { ItemFlag.HIDE_ATTRIBUTES });
        ...
        lore.clear();
        delete_meta.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "delete_ban"), PersistentDataType.STRING, "delete_ban");
        closeBan_meta.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "close_ban"), PersistentDataType.STRING, "close_ban");
