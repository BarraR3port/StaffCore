...
public class ChatManager extends MenuC {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "mcmanager"), PersistentDataType.STRING)) {
            p.closeInventory();
            (new MuteChatManager(main.getPlayerMenuUtility(p), this.plugin)).open(p);
            p.updateInventory();
            e.setCancelled(true);
        } else if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "panel"), PersistentDataType.STRING)) {
            e.setCancelled(true);
        } else if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "TStaffOn"), PersistentDataType.STRING)) {
            p.closeInventory();
            PlayerData.remove(new NamespacedKey((Plugin)this.plugin, "staffchat"));
            ...
            p.updateInventory();
            e.setCancelled(true);
        } else if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "TStaffOff"), PersistentDataType.STRING)) {
            p.closeInventory();
            PlayerData.set(new NamespacedKey((Plugin)this.plugin, "staffchat"), PersistentDataType.STRING, "staffchat");
            if (utils.mysqlEnabled())
            ...
            p.updateInventory();
            e.setCancelled(true);
        } else if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "ccmanager"), PersistentDataType.STRING)) {
            p.closeInventory();
    ...
    public void setMenuItemsPlayer(Player p) {
        ...
        metaTStaffOn.addItemFlags(new ItemFlag[] { ItemFlag.HIDE_ENCHANTS });
        metaTStaffOn.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "TStaffOn"), PersistentDataType.STRING, "TStaffOn");
        metaTStaffOff.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "TStaffOff"), PersistentDataType.STRING, "TStaffOff");
        metaTChat.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "ccmanager"), PersistentDataType.STRING, "ccmanager");
        metaMChat.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "mcmanager"), PersistentDataType.STRING, "mcmanager");
        ...
        if (p.getPersistentDataContainer().has(new NamespacedKey((Plugin)main.plugin, "staffchat"), PersistentDataType.STRING)) {
            this.inventory.setItem(20, TStaffChatOn);
        } else if (!p.getPersistentDataContainer().has(new NamespacedKey((Plugin)main.plugin, "staffchat"), PersistentDataType.STRING)) {
        ...
        this.inventory.setItem(25, redPanel());
        this.inventory.setItem(31, makeItem(Material.BARRIER, ChatColor.DARK_RED + "Close", new String[0]));
    }
}
