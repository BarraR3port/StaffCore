...
public class MutePlayer extends PaginatedMenu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)this.plugin, "mute"), PersistentDataType.STRING)) {
            p.closeInventory();
            Player jugador = p.getServer().getPlayer(e.getCurrentItem().getItemMeta().getDisplayName());
            p.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "muted_player"), PersistentDataType.STRING, jugador.getName());
        ...
        if (players != null && !players.isEmpty())
            for (int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if (players.get(this.index) != null) {
                    ItemStack p_head = utils.getPlayerHead(((Player)players.get(this.index)).getName());
                    ItemMeta meta = p_head.getItemMeta();
                    ArrayList<String> lore = new ArrayList<>();
                    meta.setDisplayName(((Player)players.get(this.index)).getName());
                    lore.add(utils.chat(utils.chat("&cMute &r") + ((Player)players.get(this.index)).getDisplayName()));
                    meta.setLore(lore);
                    meta.getPersistentDataContainer().set(new NamespacedKey((Plugin)main.plugin, "mute"), PersistentDataType.STRING, "mute");
