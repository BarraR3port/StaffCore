...
public class staffcore implements TabExecutor{
    ...
    public boolean onCommand( CommandSender sender , Command cmd , String label , String[] args ){
        if ( !(sender instanceof Player) ) {
            if ( args.length == 0 ) {
                utils.tell( sender , plugin.getConfig( ).getString( "server_prefix" ) + "&7Use &9/staffcore version/vanished/staff/reload" );
            }
            if ( args.length == 1 ) {
                if ( args[0].equalsIgnoreCase( "version" ) ) {

                    utils.tell( sender , " " );
                    utils.tell( sender , "                 &bAuthor: &5" + plugin.getDescription( ).getAuthors( ) );
                    utils.tell( sender , " " );
                ...
                utils.tell( sender , plugin.getConfig( ).getString( "server_prefix" ) + "&9/staffcore version/vanished/staff/reload" );
            }
        } else {
