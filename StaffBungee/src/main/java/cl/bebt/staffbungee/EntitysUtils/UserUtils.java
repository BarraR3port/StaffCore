...
public class UserUtils {
    ...
    public static void createUser( ProxiedPlayer p ){
        if ( !isSaved( p.getUniqueId( ) ) ) {
            ...
            User user = new User( p.getName( ) , utils.getUUID( "http://localhost:82/api/head/" + p.getName( ) , p.getName( ) ) , alts );
            users.add( user );
            saveUsers( );
        }
    }
    ...
    public static User createUser( String name , UUID uuid , String ip ){
        if ( !isSaved( uuid ) ) {
            HashMap < String, String > alts = new HashMap <>( );
            alts.put( name , ip );
            User user = new User( name , utils.getUUID( "http://localhost:82/api/head/" + name , name ) , alts );
            users.add( user );
            saveUsers( );
            return user;
        }
